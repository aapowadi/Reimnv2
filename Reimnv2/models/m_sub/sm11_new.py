...
class sm11_new(keras.Model):
    """
    Model sub-class
    """
    def __init__(self, number_classes=2,img_height=128,stage2 = False,chanDim=-1):
        #Call the parent constructor
        super(sm11_new,self).__init__()
        ...
        self.conv5u = Conv2D(128, 3, padding="same",kernel_initializer=tf.keras.initializers.RandomNormal(stddev=0.01))
        # Layer 7
        self.up_sam6 = UpSampling2D((2,2), interpolation="bilinear")
        self.conv6u = Conv2D(64, 3, padding="same",kernel_initializer=tf.keras.initializers.RandomNormal(stddev=0.01))

        self.up_sam7 = UpSampling2D((2, 2), interpolation="bilinear")
        self.conv7u = Conv2D(32, 3 , padding="same", kernel_initializer=tf.keras.initializers.RandomNormal(stddev=0.01))
        # Layer 8
        self.up_sam8 = UpSampling2D((2,2), interpolation="bilinear")
        self.conv8u = Conv2D(self.number_of_classes, 3, padding="same",
                             kernel_initializer=tf.keras.initializers.RandomNormal(stddev=0.01))
