...
class Model_10n:
    ...
    def createModel(self, X, Xd, Xpred, p_keep_conv, p_keep_hidden, img_width, img_height,phase_train):
        ...
        self.scale5 = tf.Variable(tf.ones([512]))
        self.beta5 = tf.Variable(tf.zeros([512]))

        # convolution
        conv1 = conv_layer(X, self.w1, self.p_keep_conv, self.scale1,
                           self.beta1,phase_train)
        conv2 = conv_layer(conv1, self.w2, self.p_keep_conv, self.scale2,
                           self.beta2,phase_train)
        conv3 = conv_layer(conv2, self.w3, self.p_keep_conv, self.scale3,
                           self.beta3,phase_train)
        conv4 = conv_layer(conv3, self.w4, self.p_keep_conv, self.scale4,
                           self.beta4,phase_train)
        conv5 = conv_layer(conv4, self.w5, self.p_keep_conv, self.scale5,
                           self.beta5,phase_train)
        # deconvolution
        ...
        self.wf1 = init_weights([16 * 16 * 16, 6025], "wf1")
        ...
        self.wf1_r = init_weights([16 * 16 * 16, 8025], "wf1_r")
