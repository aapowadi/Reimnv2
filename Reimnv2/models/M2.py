...
class M2(keras.Model):
    ...
    def __init__(self, number_classes=2,img_height=128,chanDim=-1):
        ...
        # Encoder
        # Layer 1
        self.conv1=Conv2D(32, (3, 3), (1, 1),padding="same",kernel_initializer=tf.random_normal_initializer(stddev=0.01))
        self.max1=MaxPooling2D(pool_size=(2, 2))
        self.bn1=BatchNormalization(axis=chanDim,epsilon=1e-3)
        # Layer 2
        self.conv2 = Conv2D(64, (3, 3), (1, 1), padding="same",kernel_initializer=tf.random_normal_initializer(stddev=0.01))
        self.max2=MaxPooling2D(pool_size=(2, 2))
        self.bn2 = BatchNormalization(axis=chanDim,epsilon=1e-3)
        # Layer 3
        self.conv3 = Conv2D(128, (3, 3), (1, 1), padding="same",kernel_initializer=tf.random_normal_initializer(stddev=0.01))
        self.max3=MaxPooling2D(pool_size=(2, 2))
        self.bn3 = BatchNormalization(axis=chanDim,epsilon=1e-3)
        # Layer 4
        self.conv4 = Conv2D(256, (3, 3), (1, 1), padding="same",kernel_initializer=tf.random_normal_initializer(stddev=0.01))
        ...
        self.conv5u = Conv2D(128,(1,1),padding="same",kernel_initializer=tf.random_normal_initializer(stddev=0.01))
        # Layer 6
        self.up_sam6 = UpSampling2D((2,2), interpolation="bilinear")
        self.conv6u = Conv2D(64,(1,1),padding="same",kernel_initializer=tf.random_normal_initializer(stddev=0.01))
        # Layer 7
        self.up_sam7 = UpSampling2D((2,2), interpolation="bilinear")
        self.conv7u = Conv2D(32,(1,1),padding="same",kernel_initializer=tf.random_normal_initializer(stddev=0.01))
        # Layer 8
