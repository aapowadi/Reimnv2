...
class Solver_nrm:
    ...
    def init(self, img_width, img_height, restore_from_file="", cont=False):
        ...
        self.Y_pre_logits, self.fcn_predictions, self.Y_pre_pose_t, self.Y_pre_pose_q = \
            self.model.createModel(self.pl_X_rgb, self.pl_X_depth, self.pl_X_pred, self.pl_keep_conv,
                                   self.pl_keep_hidden,
    ...
    def __initSolver__(self):
        ...
        self.pose_loss_q = tf.reduce_sum(tf.square(tf.abs(self.Y_pre_pose_q[:, 3] - self.pl_Y_pose_q[:, 3])) + tf.norm(
            self.Y_pre_pose_q[:, 0:3] - self.pl_Y_pose_q[:, 0:3], axis=1))
        ...
        with tf.Session() as sess:
            ...
            for i in range(self.num_epochs):
                ...
                for start, end in training_batch:
                    ...
                    if (self.stage2 == True):
                        t_rms, t_all, q_mean, q_all = eval_pose_accuracy(self.Ytr_pose[train_indices][:, 0:3],
                                                                         pr_pose_t,
                                                                         self.Ytr_pose[train_indices][:, 3:7],
                                                                         pr_pose_q)
                    else:
                        t_rms = 0;
                        t_all = 0;
                ...
                test_predict, test_prob, test_loss, test_pose_t_loss, test_pose_q_loss, test_pose_t, test_pose_q = self.__test_step(
                    sess,
                    self.Xte_rgb[test_indices], self.Xte_depth[test_indices],
                    self.Yte_mask[test_indices], self.Yte_pose[test_indices][:, 0:3],
                ...
                if (self.stage2 == True):
                    ...
                    q_all = 0;
                print(
                    "Epoch {self.step}, loss {test_loss / self.im_width*self.im_height}, avg. precison: {precision}, avg. recall: {recall}, pose t loss: {test_pose_t_loss}, pose q loss: {test_pose_q_loss}")
