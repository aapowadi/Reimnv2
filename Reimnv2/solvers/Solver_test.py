...
class Solver_test:
    ...
    def init(self, img_width, img_height, restore_from_file="", cont=False):
        ...
        self.manager_r = tf.train.CheckpointManager(self.ckpt_t, self.trained_model_path + "pose_r/", max_to_keep=3)
        # ----------------------------
        # -----  Train and Save  -----
        # ----------------------------
    ...
    def get_masks(self,indices, training = False):
        if training:
            x = tf.reshape(self.Ytr_mask[indices],(-1, self.im_height, self.im_height, 2))
            ...
            train_masks = tf.reshape(tensor=result_max, shape=(-1, self.im_height, self.im_height, 1))
            return train_masks
        else:
            x = tf.reshape(self.Yte_mask[indices], (-1, self.im_height, self.im_height, 2))
            result_max = tf.argmax(x, 3)  # max result along axis 3
            result_max_labels = tf.equal(result_max, 1)  # object class is 0
            result_max = tf.cast(result_max_labels, tf.float32)
            train_masks = tf.reshape(tensor=result_max, shape=(-1, self.im_height, self.im_height, 1))
    ...
    def train_t(self, indices, training=True):
        train_masks = self.get_masks(indices,True)
        if training:
            with tf.GradientTape() as tape:
                pose_t = self.net_t(train_masks, self.Xtr_depth[indices], self.drop_conv, training=True)
            ...
            pose_t = self.net_t(train_masks, self.Xtr_depth[indices], 0, training=False)
            loss_t = self.pose_loss_old_t(pose_t, self.Ytr_pose[indices][:, 0:3])
    ...
    def train_r(self, indices, training=True):
        ...
        if training:
            with tf.GradientTape() as tape:
                pose_r = self.net_r(train_masks, self.Xtr_depth[indices], self.drop_conv, training=True)
            ...
            pose_r = self.net_r(train_masks, self.Xtr_depth[indices], 0, training=False)
            loss_r = self.pose_loss_old_r(pose_r, self.Ytr_pose[indices][:, 3:7])
    ...
    def test_t(self, indices):
        train_masks = self.get_masks(indices, False)
        pose_t = self.net_t(train_masks, self.Xte_depth[indices], 0, training=False)
    ...
    def test_r(self, indices):
        train_masks = self.get_masks(indices, False)
        pose_r = self.net_r(train_masks, self.Xte_depth[indices], 0, training=False)
