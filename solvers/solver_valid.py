...
class solver_valid:
    ...
    def plot_acc(self):
        ...
        if (os.path.exists(self.saver_log_folder + 'seg_results.csv')):
            with open(self.saver_log_folder + 'seg_results.csv', 'r') as csvfile:
                ...
                plt.xlabel('Epoch')
                plt.ylabel('Accuracy')
                plt.title('Train-segmentation_Accuracy'+self.plot_title)
                plt.legend()
        ...
        if (os.path.exists(self.saver_log_folder + 'test_loss.csv')):
            with open(self.saver_log_folder + 'test_loss.csv', 'r') as csvfile:
                ...
                if (len(clm_val_rt_loss) > 0):
                    ...
                    plt.xlabel('Epoch')
                    plt.ylabel('Loss')
                    plt.title('Cost of training Pose'+self.plot_title)
                    plt.legend()
                    ...
                    plt.xlabel('Epoch')
                    plt.ylabel('Loss')
                    plt.title('Cost of training Segmentation'+self.plot_title)
                    plt.legend()
        ...
        if (os.path.exists(self.saver_log_folder + 'test_pose_results.csv')):
            with open(self.saver_log_folder + 'test_pose_results.csv', 'r') as csvfile:
                ...
                plt.xlabel('Epoch')
                plt.ylabel('Accuracy')
                plt.title('Train-pose_Accuracy'+self.plot_title)
                plt.legend()
                ...
                if size != 0:
                    ...
                    plt.title('Precision-recall (train-pose) -Cutoff = 0.3 m'+self.plot_title)
                    plt.legend()
                    ...
                    plt.xlabel('Epoch')
                    plt.ylabel('Accuracy')
                    plt.title('Accuracy (train-rotation_rms)'+self.plot_title)
                    plt.legend()
                    ...
                    plt.xlabel('recall')
                    plt.ylabel('precision')
                    plt.title('Precision-recall (train-rotation). Cutoff = 0.11 radian'+self.plot_title)
                    plt.legend()
    ...
    def __sample_test(self, Xte, Xte_depth, Yte, Yte_pose, sess):
        ...
        for i in range(5):
            ...
            rgb_img=Xte[idx]
            #rgb_img = rgb_img[:,1]
            # res_rgb = rgb_img.reshape([-1, self.im_width]).astype(float)
            res_rgb = rgb_img * 255
            test_mask = Yte[idx]
