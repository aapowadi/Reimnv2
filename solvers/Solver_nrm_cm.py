...
class Solver_nrm_cm:
    ...
    def init(self, img_width, img_height, restore_from_file = "",cont = False):
        ...
        #-------------------------------------------------------------------------------------
        # compact.v1.placeholders
        self.phase_train = tf.compact.v1.placeholder(tf.bool, name='phase_train')
        # the input image of size [64, self.im_width, 3]
        self.pl_X_rgb = tf.compact.v1.placeholder("float", [None, img_width, img_height, 3], name="input_feed")

        # the input image of size [img_width, self.im_width, 3]
        self.pl_X_depth = tf.compact.v1.placeholder("float", [None, img_width, img_height, 1], name="aug_map")

        # predictions from the first network, input for the second network.
        self.pl_X_pred = tf.compact.v1.placeholder("float", [None, img_width, img_height, 1])

        # vector for the output data.
        # The network generates an output layer for each class.
        # self.im_width*self.im_height = 64 * 64
        self.pl_Y_mask = tf.compact.v1.placeholder("float", [None, img_width * img_height, self.number_classes], name="Y_mask")

        # the pose output ground truth
        self.pl_Y_pose_t = tf.compact.v1.placeholder("float", [None, self.number_pose_t_outputs], name="Y_pose_t")

        # the pose_q output ground truth
        self.pl_Y_pose_q = tf.compact.v1.placeholder("float", [None, self.number_pose_q_outputs], name="Y_pose_q")

        # Dropout ratio compact.v1.placeholder
        self.pl_keep_conv = tf.compact.v1.placeholder("float", name="keep_conv")
        self.pl_keep_hidden = tf.compact.v1.placeholder("float", name="keep_hidden")
        ...
        self.Y_pre_logits, self.fcn_predictions, self.Y_pre_pose_t, self.Y_pre_pose_q = \
            self.model.createModel(self.pl_X_rgb, self.pl_X_depth, self.pl_X_pred, self.pl_keep_conv, self.pl_keep_hidden,
            ...
            for i in range(self.num_epochs):
                ...
                for start, end in training_batch:
                    ...
                    self.Xtr_rgb[train_indices], self.Xtr_depth[train_indices], self.Ytr_mask[train_indices],
                                         self.Ytr_pose[train_indices][:,0:3], self.Ytr_pose[train_indices][:,3:7],
                                                                                               self.keep_conv, self.keep_hidden)
