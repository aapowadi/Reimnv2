...
class Solver2StageRGBD_6DoF:
    ...
    def init(self, img_width, img_height, restore_from_file = ""):
        ...
        self.Y_pre_logits, self.fcn_predictions, self.Y_pre_pose_t, self.Y_pre_pose_q = \
            self.model.createModel(self.pl_X_rgb, self.pl_X_depth, self.pl_X_pred, self.pl_keep_hidden, self.pl_keep_hidden, img_width, img_height)
    ...
    def __initSolver__(self):
        ...
        self.train_pose_t = tf.train.RMSPropOptimizer(self.learning_rate, 0.1).minimize(self.pose_loss_t)
        ...
        self.train_pose_q = tf.train.AdamOptimizer(self.learning_rate, 0.1).minimize(self.pose_loss_q)
    ...
    def __start_train(self):
        ...
        with tf.Session() as sess:
            ...
            for i in range(self.num_epochs):
                ...
                print(f"Epoch {self.step}, loss {test_loss / 4096}, avg. precison: {precision}, avg. recall: {recall}, pose t loss: {test_pose_t_loss}, pose q loss: {test_pose_q_loss}")
                ...
                file_str = str(self.step) + "," + str(test_loss/ 4096) + "," + str(precision) + "," + str(recall) + "," + str(test_pose_t_loss)+ "," + str(test_pose_q_loss) + "\n"
                file.write(file_str)
                file.close()
    ...
    def __start_eval(self, Xte_rgb, Xte_depth, Yte_mask, Yte_pose):
        ...
        with tf.Session() as sess:
            ...
            average_t_loss = 0
            average_q_loss = 0
            t_precis=0        # translation precision
            q_precis=0        # Orientation precision
            i = 0
            ...
            for start, end in validation_batch:
                ...
                print(f"Batch {i}, seg. loss {loss / 4096}, seg. precison: {precision}, seg. recall: {recall}, pose t loss: {t_loss}, pose q loss: {q_loss}")
                ...
                average_loss = average_loss + loss / 4096
    ...
    def __getAccuracy(self, Y, Ypr, validation="", start_index=0):
        ...
        """
        pr = Ypr.reshape([-1, 64, 64]).astype(float)
        pr = pr * 255

        y = Y[:, :, 1]  # this is the background mask. But the buny model is = 0 here.
        y = y.reshape([-1, 64, 64]).astype(float)
        y = y * 255
    ...
    def __sample_test(self, Xte, Xte_depth, Yte, Yte_pose, sess):
        ...
        pr = pr.reshape([-1, 64]).astype(float)
        pr = pr * 255
        test_mask = Yte[idx]
        test_mask = test_mask[:, 1]
        te = test_mask.reshape([-1, 64]).astype(float)
        te = te * 255
