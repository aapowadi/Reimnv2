...
class Model_4n2:
    ...
    def createModel(self, X, Xd, Xpred, p_keep_conv, p_keep_hidden, img_width, img_height,phase_train):
        """
        ...
        # convolution
        conv1 = conv_layer(X, self.w1, self.p_keep_conv,phase_train)
        conv2 = conv_layer(conv1, self.w2, self.p_keep_conv,phase_train)

        # deconvolution
        deconv3 = upsample_layer(conv2, 64, "deconv3", upscale)
        pconv3 = tf.nn.conv2d(deconv3, init_weights([1, 1, 64, 32], "pw3"), strides=[1, 1, 1, 1], padding='SAME')
        conv_n3 = tf.keras.layers.BatchNormalization()(pconv3, training=phase_train)
        deconv4 = upsample_layer(conv_n3, 32, "deconv4", 2)
        result = tf.nn.conv2d(deconv4, init_weights([1, 1, 32, self.number_of_classes], "pw4"), strides=[1, 1, 1, 1],
                              padding='SAME')
        # result_n = tf.keras.layers.BatchNormalization()(result, training=phase_train)
        ...
        conv4t = conv_layer(depth_in, self.w4t, self.p_keep_conv,phase_train)
        conv5 = conv_layer(conv4t, self.w5t, self.p_keep_conv,phase_train)
        ...
        output_layer_n = tf.keras.layers.BatchNormalization()(output_layer, training=phase_train)
        output_layer_d = tf.nn.dropout(output_layer_n, self.p_keep_hidden)

        self.trans_predictions = tf.add(tf.matmul(output_layer_d, self.wout), self.bout)
        ...
        conv4r = conv_layer(depth_in, self.w4r, self.p_keep_conv,phase_train)
        conv5r = conv_layer(conv4r, self.w5r, self.p_keep_conv,phase_train)
        ...
        output_layer_rn = tf.keras.layers.BatchNormalization()(output_layer_r, training=phase_train)
        output_layer_rd = tf.nn.dropout(output_layer_rn, self.p_keep_hidden)

        self.rot_predictions = tf.add(tf.matmul(output_layer_rd, self.wout_r), self.bout_r)
