...
class m_1_norm:
    ...
    def createModel(self, X, Xd, Xpred, p_keep_conv, p_keep_hidden, img_width, img_height,phase_train):
        ...
        self.w5 = init_weights([3, 3, 256, 512], "w5")

        # convolution
        conv1 = conv_layer(X, self.w1, self.p_keep_conv,phase_train)
        conv2 = conv_layer(conv1, self.w2, self.p_keep_conv,phase_train)
        conv3 = conv_layer(conv2, self.w3, self.p_keep_conv,phase_train)
        conv4 = conv_layer(conv3, self.w4, self.p_keep_conv,phase_train)
        conv5 = conv_layer(conv4, self.w5, self.p_keep_conv,phase_train)
        # deconvolution
        deconv1 = upsample_layer(conv5, 512, "deconv0", upscale)
        pconv0 = tf.nn.conv2d(deconv1, init_weights([1, 1, 512, 256], "pw0"), strides=[1, 1, 1, 1], padding='SAME')
        deconv1 = upsample_layer(pconv0, 256, "deconv1", upscale)
        ...
        self.w4t = init_weights([3, 3, 1, 32], "w4t")  # 128 x 128 x 1 ->
        self.w5t = init_weights([3, 3, 32, 16], "w5t")  # 64 x 64 x 32 -> 32 x 32 x 4
        ...
        self.w4r = init_weights([3, 3, 1, 32], "w4r")  # 128 x 128 x 1 -> 64 x 64 x 32
        self.w5r = init_weights([3, 3, 32, 16], "w5r")  # 64 x 64 x 32 -> 32 x 32 x 4
