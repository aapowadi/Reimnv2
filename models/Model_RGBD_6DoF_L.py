...
class Model_RGBD_6DoF_L:
    ...
    June 29, 2019, RR
    - Changed the number of fully connected neurons from 825 to 625 for the rotation estimation

    July 14, 2019, RR
    - Copied this file from the original network Model_RGBD-6DoF_L for large.
    - I extended the network size. Goal is to test whether a larger network can find the transducer.
    ...
    def createModel(self, X, Xd, Xpred, p_keep_conv, p_keep_hidden):
        """
        ...
        self.w1 = init_weights([3, 3, 3, 64], "w1")
        self.w2 = init_weights([3, 3, 64, 128], "w2")
        self.w3 = init_weights([3, 3, 128, 256], "w3")
        ...
        # deconvolution
        deconv1 = upsample_layer(conv3, 256, "deconv1", upscale)
        ...
        deconv3 = upsample_layer(pconv2, 64, "deconv3", 2)
        result = tf.nn.conv2d(deconv3, init_weights([1, 1, 64, self.number_of_classes], "pw3"), strides=[1, 1, 1, 1],
                              padding='SAME')
        ...
        self.segmentation_logits = tf.reshape(tensor=result, shape=(-1, 4096, self.number_of_classes))
        ...
        self.fcn_predictions = tf.reshape(tensor=result_max, shape=(-1, 64, 64, 1))
        ...
        self.w4 = init_weights([3, 3, 1, 32], "w4") # 64 x 64 x 1 -> 32 x 32 x 32
        self.w5 = init_weights([3, 3, 32, 4], "w5") # 32 x 32 x 32 -> 16 x 16 x 4
        self.wf1 = init_weights([16 * 16 * 4, 625], "w6")
        ...
        conv4 = conv_layer(depth_in, self.w4, self.p_keep_conv)
        conv5 = conv_layer(conv4, self.w5, self.p_keep_conv)
        ...
        self.wf1_r = init_weights([16 * 16 * 4, num_inner_nodes], "w6r")
        self.bf1_r = tf.Variable(tf.zeros(num_inner_nodes))
        self.wout_r = init_weights([num_inner_nodes, self.number_rot_outputs], "w_out_r")
        ...
        FC_layer_r = tf.reshape(conv5r, [-1, self.wf1.get_shape().as_list()[0]])
